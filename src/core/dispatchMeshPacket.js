...
import { tryDecodeBuf } from '../packets/decodePacket.js';
    ...
    switch (getPortName(portnum)) {
        ...
        case 'NODEINFO_APP': { // NodeInfo
                ...
                if (user) { dispatchPacket ({
                    ...
                    meta: getBaseMeta(data),
                })} else console.warn('[decodeMeshPacket] Failed to decode NodeInfo:');
        ...
        case 'TELEMETRY_APP': { // Telemetry
            ...
            if (telemetry) { dispatchPacket ({
                ...
                meta: { ...getBaseMeta(data) }
            })} else console.warn('[decodeMeshPacket] Failed to decode Telemetry:');
            break;
        }

        default: {
            console.warn(`[decodeMeshPacket] Unknown port ${portnum} on channel ${getChannel(packet)}, skipping`);
            break;
        }
    }
};
