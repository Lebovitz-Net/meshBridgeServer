...
export const insertMessage = (msg) => {
  ...
  db.prepare(`
    INSERT INTO messages (message_id, channel_id, fromNodeNum, message, timestamp)
    VALUES (?, ?, ?, ?, ?)
  `).run(msg.message_id, msg.channel, msg.fromNodeNum, msg.message, msg.timestamp || Date.now());
};
